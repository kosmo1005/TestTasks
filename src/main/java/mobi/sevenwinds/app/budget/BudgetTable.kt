...
class BudgetEntity(id: EntityID<Int>) : IntEntity(id) {
    ...
    fun toResponseForAdded(): BudgetRecordReq {
        return BudgetRecordReq(year, month, amount, type,authorId)
    }
    fun toResponse(): BudgetRecordResp {
        return BudgetRecordResp(year, month, amount, type, author?.fullName, author?.createdAt)
    }
}